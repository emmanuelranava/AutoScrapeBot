            ...
            for x in div_group:
                ...
                if link_url:
                    ...
                    if collection.find_one({"link": offer_url}):
                        print(f"L'URL {offer_url} existe déjà dans la base de données, elle sera ignorée.")
                        continue
...
def send_alert_email(count):
    ...
    if recipients:
        ...
        body = f"Le scraping est terminé ! {count} nouvelles données ont été ajoutées à la base de données."
